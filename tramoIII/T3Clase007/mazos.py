...
from cartas import CartaPoker,CartaEspaniola,Carta
import random

class Mazo(ABC):
    """Clase que representa un mazo de cartas"""

    def __init__(self, mazo:'Mazo',con_cartas: bool = False, tapado: bool = False) -> None:
        """
        Inicializa el maz.
    ...
    def __next__(self):
        if self.__index < len(self):
            carta = self._MazoPoker__cartas[self.__index]
            self.__index += 1
            return carta
        else:
            raise StopIteration
...
class MazoBlackJack(Mazo):
    ...
    def llenar(self, tapado: bool = False) -> None:
        """Llena el mazo con las 520 cartas de Poker."""
        self.cartas.clear()
        for m in range(10):
            for n in range(1, 14):
                for p in range(1, 5):
